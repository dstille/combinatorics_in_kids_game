...
class Game(ABC):
    @abstractmethod
    def start():
        players, what_to_check, more  = set_up_players(get_random())
        game(players, what_to_check)

    @abstractmethod
    def get_random():
        return None

    def game(what_to_check):
        print('playing')
        print('checking', what_to_check)

class SampleGame(Game):
    def start(self):
        pl, what = self.set_up_players()
        self.game(pl, what)
    
    def set_up_players(self, var_in):
        return ['a','b','c'], 'b'
    
    def game(self, players, what_to_check):
        print('checking:', what_to_check)
